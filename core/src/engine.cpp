...
std::int64_t notional(std::uint64_t price, std::uint64_t quantity) {
  const auto value = (static_cast<__int128>(price) * quantity) / kScale;
...
void MatchingEngine::publish_depth(std::uint64_t price, std::uint8_t side) {
  ...
  if (w - read.load(std::memory_order_acquire) >= kQueueCapacity) return;
  shared_memory_->depth_queue[w % kQueueCapacity] = DepthDelta{price, level_quantity(price, side), side, {}};
...
void MatchingEngine::process_order(const OrderPacket& input) {
  ...
  if (order.side == static_cast<std::uint8_t>(OrderSide::Buy)) {
    ...
    if (order.quantity > 0 && bid_count_ < bids_.size()) { bids_[bid_count_++] = order; publish_depth(order.price, order.side); }
  } else {
    ...
    if (order.quantity > 0 && ask_count_ < asks_.size()) { asks_[ask_count_++] = order; publish_depth(order.price, order.side); }
  }
}
